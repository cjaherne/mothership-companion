  ...
  return (
    <div className="rounded-lg border border-neutral-800">
      <button
        ...
        className="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-neutral-400 hover:text-neutral-300"
      >
        <span>Run state</span>
        <span className="text-xs">{isOpen ? "−" : "+"}</span>
      </button>
      {isOpen && (
        <div className="space-y-4 border-t border-neutral-800 px-4 py-3 text-sm">
          ...
          {state.interactedNpcIds.length > 0 && (
            <div>
              ...
              <p className="text-neutral-400">{state.interactedNpcIds.join(", ")}</p>
            </div>
          )}
            ...
            <div>
              ...
              <ul className="space-y-1">
                {Object.entries(state.npcAttributeState).map(([npcId, attrs]) => (
                  <li key={npcId} className="text-neutral-400">
